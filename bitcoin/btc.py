    ...
    def FunctionBeforeMarketClose(self):
        ...
        if self.Securities['SPY'].Close > self.indicators['SPY']['SMA']['Period-200'].Current.Value:
            if self.Securities['BTCUSD'].Close > self.indicators['BTCUSD']['SMA']['Period-20'].Current.Value:
                self.SetHoldings('BTCUSD', 1, True)
            else:
                self.Liquidate()
        else:
            if self.Securities['BTCUSD'].Close > self.indicators['BTCUSD']['SMA']['Period-200'].Current.Value:
                self.SetHoldings('BTCUSD' , 1 , True)
            else:
                if self.indicators['BTCUSD']['RSI']['Period-10'].Current.Value < 25:
                    self.SetHoldings('BTCUSD' , 1 , True)
                else:
                    if self.Securities['BTCUSD'].Close < self.indicators['BTCUSD']['SMA']['Period-20'].Current.Value:
                        self.SetHoldings('BTCUSD' , -1 , True)
