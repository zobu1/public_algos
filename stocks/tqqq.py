    ...
    def FunctionBeforeMarketClose(self):
        ...
        if self.Securities['SPY'].Close > self.indicators['SPY']['SMA']['Period-200'].Current.Value:
            if self.indicators['TQQQ']['RSI']['Period-10'].Current.Value > 79:
                ...
                if self.indicators['SPXL']['RSI']['Period-10'].Current.Value > 79:
                    self.SetHoldings('UVXY', 1, True)
                else:
                    if self.indicators['QQQ']['RSI']['Period-10'].Current.Value > 80:
                        asset = self.sortEquitiesByIndicator(['SQQQ', 'SOXS', 'TLT'], 'RSI', 5)[-1]
                        self.TLT(asset)
                    else:
                        if self.indicators['TQQQ']['STD']['Period-10'].Current.Value > 5:
                            self.Liquidate()
                        else:
                            self.SetHoldings('TQQQ', 1, True)
        else:
            if self.indicators['TQQQ']['RSI']['Period-10'].Current.Value < 31:
                asset = self.sortEquitiesByIndicator(['TQQQ', 'SOXL'], 'RSI', 5, reverse=True)[-1]
                self.SetHoldings(asset, 1, True)
            else:
                if self.indicators['SPY']['RSI']['Period-10'].Current.Value < 30:
                    ...
                    if self.indicators['UVXY']['RSI']['Period-10'].Current.Value > 74:
                        if self.indicators['UVXY']['RSI']['Period-10'].Current.Value > 84:
                            asset = self.sortEquitiesByIndicator(['SQQQ', 'SOXS', 'TLT'], 'RSI', 5, reverse=True)[-1]
                            self.TLT(asset)
                        else:
                            self.SetHoldings('UVXY', 1, True)
                    else:
